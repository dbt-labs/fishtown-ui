...
const components = readdirSync(resolve(directory), { withFileTypes: true })
  ...
  .map(
    (dir) =>
      `export * from './${
        isDefaultDirectory ? join(componentPath, dir.name) : dir.name
      }';`
  );

console.log(
  `Discovered ${components.length} component directories in ${directory}`,
  '\n'
);
console.log(components.join('\n'), '\n');
console.log(`Creating index.ts file in ${isDefaultDirectory ? './' : directory}`);
writeFileSync(
  resolve(isDefaultDirectory ? './index.ts' : `${directory}/index.ts`),
  `${components.join('\n')}\n`
);
